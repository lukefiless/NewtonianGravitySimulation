    ...
    static string[] GetGravityInfo(Vector3 point, CelestialBody ignore = null)
    {
        ...
        foreach (CelestialBody body in bodies)
        {
            if (body != ignore)
            {
                ...
                var sqrDist = offsetToBody.sqrMagnitude;
                float dst = Mathf.Sqrt(sqrDist);
                var dirToBody = offsetToBody / Mathf.Sqrt(sqrDist);
                var acceleration = Universe.gravitationalConstant * body.mass / sqrDist;
